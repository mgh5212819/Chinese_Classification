...
class MLData(object):
    ...
    def preprocessor(self):
        ...
        self.dev = pd.read_csv(config.root_path + '/data/dev.csv',
                               sep='\t').dropna()
        if self.debug_mode:
            self.train = self.train.sample(n=3000).reset_index(drop=True)
        ...
        self.em.stopWords = []
        with open(root_path + '/data/stopwords.txt', 'r') as f1:
            for ip in f1.readlines():
                self.em.stopWords.append(ip.strip('\n'))
